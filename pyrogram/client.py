        ...
        if plugins.get("enabled", True):
                                    ...
                                    for handler, group in getattr(
                                        ...
                                        if isinstance(
                                            ...
                                            if (
                                                ...
                                                exclude_handlers[module_path].remove(
                                                    name
                                                )
                                                log.warning(
                                                    f'[{self.name}] [LOAD] Ignoring function "{name}" from group {group} in "{module_path}"'
                                                )
                                                continue
            ...
            for module in exclude_handlers:
                for handler in exclude_handlers[module]:
                    log.warning(
                        f'[{self.name}] [LOAD] Ignoring non-existent function "{handler}" from "{module}"'
                    )
